...
const HeaderNav = () => {
  ...
  const allViews = [
    ...
    { cat: "usingoracle", title: "Using oracle data" },
  ...
  return (
    <>
      <Menu isOpen={menuOpen} onStateChange={(hu) => handleStateChange(hu)}>
        {allViews.map((thisView, i) => {
          return (
            <Link
              key={i}
              onClick={() => handleClick()}
              ...
              className={
                location.pathname === "/" + thisView.cat ? "selectedPage" : ""
              }
            >
              {thisView.title}
            </Link>
      ...
      <div className="Header">
        <div className="Header__Nav">
          <Link
            to="/live"
            className={
              activeVoteItems > 0 ? "counter hasitems" : "counter noitems"
            }
          >
            <p>{activeVoteItems}</p>
          </Link>
